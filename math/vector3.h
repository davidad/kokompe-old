...
class _vector3
{
	...
	public:
		...
		bool operator==(const _vector3& vec) const
		{
			return (vals[0] == vec[0]) &&
					(vals[1] == vec[1]) &&
					(vals[2] == vec[2]);
		...
		static _vector3< scalar >& cast(scalar* array)
		{
			return *((_vector3*)(array));
		}
...
template<typename scalar>
_vector3< scalar > operator*(const scalar& s, const _vector3< scalar >& v)
{
	return v * s;
}
