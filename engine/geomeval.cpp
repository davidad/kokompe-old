...
int main(int argc, char* argv[]) {
  ...
  interval1.set_real_interval(-2.3, 2.3);
...
cout << "marking clauses.\n";
  ...
  for(xp = -1;xp < 1; xp+=0.05) {
	...
	octree.path(&path);
	path->fill_svg(&svg, &svg_length);
	fstream svg_file ("output.svg", ios::binary | ios::out | ios::trunc);
	svg_file.write(svg, svg_length);
	svg_file.close();
